                    ...
                    <tbody>
                        {
                           Array.isArray(services) && services.map((data, index) => {
                                return (
                                    <tr key={index}>
                                        <td>{data?.id}</td>
                                        <td><img height="120px" width="150px" src={data?.img} alt="" /></td>
                                        <td>{data?.name}</td>
                                        <td>{data?.description.slice(0,30)}.....</td>
                                        <td>
                                             <button className='btn btn-info' data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={()=>show(data)}>view</button>
                                        <button className='btn btn-success mx-2' onClick={()=>{redirect(`/editservice/${data?.id}`)}}>edit</button>
                                        <button className='btn btn-danger' onClick={()=>{dispatch(deletedata(data.id)).then(()=>{dispatch(readdata())})}}>delete</button>
                                        </td>
                                    </tr>
                                )
                            })
                                ...
                                <div className="col-md-10 mx-auto wow slideInUp" data-wow-delay="0.1s">
                                    <div className="blog-item bg-dark rounded overflow-hidden">
                                        <div className="row">
                                            ...
                                            <div className="p-4">
                                                <h4 className="mb-3 text-center text-capitalize text-light">{view.name.slice(0,15)}..</h4>
                                                <h3 className='ms-3 f-6 text-light'> {view.description}...   </h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
